...
public class KWDESUtil {
    ...
    private static long BitTransform(int[] array, int len, long source) {
        ...
        for (bti = 0; bti < len; bti++) {
            if (array[bti] >= 0 && (source & g_arrayMask[array[bti]]) != 0) {
    ...
    private static void DESSubKeys(long key, long[] K, int mode) {
        int j;
        /* PC-1变换 */
        long temp = BitTransform(g_arrayPC_1, 56, key);
        for (j = 0; j < 16; j++) {
            /* 循环左移 */
            {
                long source = temp;
    ...
    private static long DES64(long[] subkeys, long data) {
        ...
        long out = BitTransform(g_arrayIP, 64, data);
        ...
        out = ((long) pSource[1] << 32 & 0xffffffff00000000L) | 0x00000000ffffffffL & pSource[0];
    ...
    public synchronized static byte[] encrypt(byte[] src, int srcLength, byte[] key, int keyLength) {
        ...
        int temp = 0;
        for (long aPEncyrpt : pEncyrpt) {
            for (int j = 0; j < 8; ++j) {
                result[temp] = (byte) (0xff & (aPEncyrpt >> (j * 8)));
                temp++;
            }
        }
